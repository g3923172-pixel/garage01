var db = new Dexie("GarageMasterDB");
...
async function seedUsers() {
    try {
        ...
        console.log('[SEED] Users count:', count);
        if (count === 0) {
        ...
        alert('Error creating admin user: ' + e.message + '\n\nPlease check if RLS is disabled on your Supabase users table.');
    }
}
...
async function checkAuth() {
    ...
    if (sessionUser) {
        ...
        } catch(e) { console.warn('Journal handle restore skipped:', e); }
...
async function handleLogin(e) {
    ...
    console.log('[LOGIN] Attempting login for:', u);
    const user = await db.users.where('username').equals(u).first();
    console.log('[LOGIN] User found from DB:', user);
    ...
    if (user && user.password === p) {
        ...
        try { await attemptFolderReauth(); } catch(e) { console.warn('Folder reauth skipped:', e); }
        ...
        console.error('[LOGIN] Login failed. User:', user, 'Password match:', user ? user.password === p : 'no user');
...
async function autoLoadGrnPrice(itemId) {
    if (!itemId) return;
    const part = await db.inventory.get(itemId);
    if (part) {
...
(async () => {
    try {
        console.log('[INIT] Starting seedUsers...');
        await seedUsers();
        console.log('[INIT] seedUsers complete. Starting checkAuth...');
        await checkAuth();
        console.log('[INIT] checkAuth complete.');
    } catch(e) {
